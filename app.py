...
def _cache_put(comparison_id: str, words1: list, words2: list) -> None:
    with _diff_cache_lock:
        _diff_cache[comparison_id] = (words1, words2)
    ...
    if cached is None:
        return jsonify({
            ...
            "ai_summary_error": (
                ...
                "It may have already been summarized or the server was restarted."
            )
        }), 404

    words1, words2 = cached

    try:
        summary = generate_change_summary(words1, words2)
        ...
        return jsonify({
            "ai_summary": None,
            "ai_summary_error": f"Unexpected summarizer error: {e}"
        })
# ─────────────────────────────────────────────────────────────────────────────
